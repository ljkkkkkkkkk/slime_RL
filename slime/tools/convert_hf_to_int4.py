...
def parse_args():
    ...
    parser.add_argument("--data-dir", dest="local_data_path", type=str, required=True, help="dataset path")
...
def main():
    ...
    num_bits = 4 if "W4" in args.quant_type else 8
    ...
    recipe = GPTQModifier(
        ...
        scheme={
            "weights": {
                "num_bits": num_bits,
                "type": "int",
                "symmetric": False,     # 非对称量化通常精度更高
                "strategy": "tensor",   # 整个张量共用缩放系数
                "group_size": args.quant_group_size # 使用你命令行传入的 32
            }
        },
